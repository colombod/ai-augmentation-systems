...
correctly once invoked." Full S-6/S-7 classification logic, the complete briefing assembler,
...
The same move here: `agents/chief-of-staff.md` built thin enough to do S-5's citation check
...
the `FR-48` fallback), because S-5's `FR-20`, S-10 entirely, and S-8's queue-reading all read
or write it. S-5/S-6/S-7's complete triage logic (Phase 7) builds directly on that. S-10
...
executes everything serially regardless. S-8 (Phase 8) needs both: real S-5/S-6/S-7 survivors
to assemble, and S-10 to exist at all before `FR-49`'s merge case is even testable. Last, the
...
the whole S-5–S-8+S-10 mechanism is real rather than after Phase 5's own thin proof.
...
| Work item | Size | Confidence | Depends on |
| :-- | :-- | :-- | :-- |
| Walking skeleton: `agents/chief-of-staff.md` (real S-5 citation check, stubbed rest), pointer section on exactly 2 consulting-agent files | S | high | Phase 4 |
...
clause). CoS-2 can slip to just before Phase 8, since only `FR-47`'s concurrent-ordering half
...
(`FR-20`, `FR-52`), mission capture's storage half (Interface 4), queue scaffolding
(Interface 3), and the epic-wide fallback (`FR-48`).
...
| `templates/mission.md` + capture mechanism (capture half only — S-10's drift-check logic is Phase 7b) | S | high (plain markdown; architecture names this as needing no spike) | — |
| `templates/chief-of-staff-queue.md` + queue read/write scaffolding | S | high (plain markdown, no new mechanism) | — |
| `FR-48` fallback path, built and example-based-tested | S | high | subagent above |

**Verification in this phase:** example-based fixtures, per architecture's Test strategy
table's `FR-48` row and decision-log format/schema row.
...
### Phase 7: S-5/S-6/S-7 complete triage logic

**Entry criteria:** Phase 6 complete.
**Delivers:** `FR-17`–22, `FR-24`–28, `FR-50`.
...
/ no-source-at-all (S-5); traces-to-requirement vs no-trace (S-6); real-execution-answerable
vs operator-authority-required (S-7) — each producing the documented behavior.

| Work item | Size | Confidence | Depends on |
| :-- | :-- | :-- | :-- |
| Complete S-5 (upgrade Phase 5's stub: stale-reflag case, `FR-20` decision-log wiring) | S | high (core logic already proved live in Phase 5) | Phase 6 |
| Build S-6 (bounce classification, messaging, twice-bounced escalation, full Case table) | M | medium | Phase 6 |
| Build S-7 (technical-unknown classification, spike citation, split-question handling, unclaimed-spike surfacing) | L | medium (qa-strategist's own table rates most of this "High" risk; more Case-table branches than S-5/S-6) | Phase 6 |

**Verification in this phase:** example-based + integration, per architecture's Test-strategy
table rows for S-5/S-6/S-7.
**Cut list:** `FR-27` (matching-spike citation) and `FR-50` (unclaimed-spike surfacing) drop
...
### Phase 7b: S-10 mission capture + drift-check — parallel with Phase 7
...
other.
**Delivers:** `FR-37`–40, `FR-52`.
**Demonstrable exit:** a fixture where output passes S-6 (traces to a stated requirement) but
...
| Work item | Size | Confidence | Depends on |
| :-- | :-- | :-- | :-- |
| S-10 drift-check logic + full Case table (legitimate-evolution resolution, recapture, zero-state) | M | medium | Phase 6 |
| `FR-52` — wrong drift flag recorded in the decision log | S | high (reuses Phase 6's decision-log mechanism; same trigger-then-log pattern as `FR-20`) | above, Phase 6 |

**Verification in this phase:** example-based + integration, per architecture's Test-strategy
table.
**Cut list:** `FR-52` drops first — "should," not "must." The recapture Case-table row
(revision-history table, one decision-log entry) is not cuttable: it is this epic's own
evidence trail for whichever way product-owner eventually answers Open Question 9.

### Phase 8: S-8 briefing assembly + FR-49 merge

**Entry criteria:** Phase 7 **and** Phase 7b both complete — S-8 needs real S-5/S-6/S-7
survivors to assemble, and `FR-49`'s merge case is not even testable until S-10 exists.
**Delivers:** `FR-29`–32, `FR-45`–47, `FR-49`, `FR-51`.
...
pull; an output flagged by both S-6 and S-10 routes as one merged item, not two.

| Work item | Size | Confidence | Depends on |
| :-- | :-- | :-- | :-- |
| Briefing accumulation, ranking, default/no-default marking (`FR-29`–32) | L | medium (9 FRs under one scenario; architecture calls this "the scenario's core... guarantee") | Phase 7, Phase 7b |
| Push exception (`FR-45`/46) + mid-exchange pause/resume (`FR-47`'s pause/marker half) | M | medium | above |
| `FR-47`'s concurrent-arrival ordering half | S | medium — inherits Spike CoS-2's confirmed batching assumption; not tested ahead of it | Phase 5 (CoS-2) |
| `FR-49` merge integration test | S | high (structural read-before-insert check, already specified in architecture's Interface 2) | Phase 7, Phase 7b |
| `FR-51` grouped/summarized briefing at scale | S | high (reuses S-1's already-shipped grouping convention) | above |

**Verification in this phase:** integration (`FR-29`, `FR-47`'s pause/resume) + example-based
(`FR-30`/31/45/46/49/51), per architecture's table.
**Cut list:** `FR-51` (grouped/summarized display) drops first — "should," same precedent the
harden epic set for its own Phase 2 ("a working, plain report ships before a well-formatted
one"). `FR-47`'s concurrent-ordering half stays gated on CoS-2 regardless of pressure — testing
it ahead of a confirmed batching assumption tests the wrong thing.

### Phase 9: full 9-agent pointer-section rollout

**Entry criteria:** Phase 8 complete — the whole S-5–S-8+S-10 mechanism is real, not just
...
### Phase 10: Stage-2 — S-9, S-11 — named future phase, not built now

Per `prd.md`'s own staging (S-9, S-11 explicitly deferred) and the same precedent the harden
epic set for its own Stage 2 (`FR-13`–16): not entered until product-owner opens it. S-9's own
**Grounding: assumed** — zero data exists until Phases 5–9 ship and produce real, logged
S-5–S-8 outcomes to learn from, the same ledger-before-consumer sequencing the harden epic
...
**Delivers if opened:** `FR-33`–36 (S-9 learned shortcuts), `FR-41`–44 (S-11 repo-doc gap
checks).
...
```
Phase 5 (CoS-1, CoS-2)
   └─ Phase 6 (subagent + decision log + mission.md + queue.md + FR-48)
        ├─ Phase 7  (S-5/S-6/S-7 triage)   ─┐
        └─ Phase 7b (S-10, parallel)        ├─ Phase 8 (S-8 + FR-49 merge) ─ Phase 9 (9-agent rollout) ─ Phase 10 (Stage-2, not built)
```
...
`FR-23` does not appear in `prd.md`'s FR table at all — retired before this pass, not a gap
introduced here.

| FR | Phase | Notes |
| :-- | :-- | :-- |
| FR-17–19 | Phase 7 | Core citation-or-nothing logic; exercised in thin form by Phase 5's own walking skeleton first |
| FR-20 | Phase 7 | Needs Phase 6's decision log to exist |
| FR-21, FR-22 | Phase 7 | S-6 bounce classification + messaging |
| FR-24 | Phase 7 | Twice-bounced escalation |
| FR-25–28 | Phase 7 | S-7 classification, spike routing, split-question handling |
| FR-50 | Phase 7 | Unclaimed-spike surfacing — "should," see Phase 7's cut list |
| FR-29–32 | Phase 8 | S-8 briefing accumulation, ranking, default logic |
| FR-45–46 | Phase 8 | Push exception |
| FR-47 | Phase 8 | Pause/resume half in Phase 8; concurrent-ordering half inherits Spike CoS-2 (Phase 5) |
| FR-48 | Phase 6 | Built and tested here; re-exercised across all 9 agents at Phase 9's rollout |
| FR-49 | Phase 8 | Needs both Phase 7 (S-6) and Phase 7b (S-10) |
| FR-51 | Phase 8 | "Should," see Phase 8's cut list |
| FR-37–40 | Phase 7b | S-10 mission capture + drift-check |
| FR-52 | Phase 7b | "Should," see Phase 7b's cut list |

**Deferred, Stage-2, Phase 10 (not built this round):** `FR-33`–36 (S-9, learned shortcuts —
`prd.md`'s own staging, zero data exists until Phases 5–9 ship), `FR-41`–44 (S-11, repo-doc
...
| S-10's broad trigger ("whenever new output is produced anywhere") applied literally makes every artifact write pay a mission-drift check, at real cost | Medium | Medium | Named, not solved — exact trigger granularity is a story-time scoping call | delivery-lead | Phase 7b story-scoping, before build starts |
| Two subagents dispatched in parallel both resolve a blocking item and both attempt to update `queue.md` near-simultaneously | Low | Medium if it happens — a lost update | Spike CoS-2 confirms batching; the `FR-49` merge check (read-before-insert) shares the same read-modify-write discipline | solution-architect | Phase 5's CoS-2 result; re-check if Phase 8's merge check ever fires unexpectedly |
