...
| FR-18 | Sprint verdict maps to the existing three-way rubric — no fourth verdict. A story is **debt-tainted** if it is itself `non-convergent`/unresolved-`irreducible`, or if it consumes a declared `outputs=` from a debt-tainted story (checked transitively, not just one hop — architecture `ADR-013`). **Not accepted** if any `done` story is debt-tainted. **Accepted with debt** if a debt-tainted story exists but no `done` story depends on it (including the case where it declared no `outputs=` at all — by construction nothing can be silently relying on it) (decided 2026-08-13, refined 2026-08-13 per `ADR-013` to cover a chained debt-tainted-consumes-debt-tainted case the original two-clause rule left silent) | S-6 | must |
