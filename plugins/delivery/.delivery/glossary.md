...
| Documented interface | The specific, named source that counts as a dependency plugin's stable public contract for compilation purposes — for `attractor`, this means `plugins/attractor/README.md` specifically, not its engine source, tests, or internal build status. | `attractor-handoff/brief.md`'s non-goal; `prd.md` `FR-6`; `architecture.md`; `ADR-011` | — | "documented," used loosely — this term is the load-bearing, precise form; used at least once, this initiative selectively applied "not in the README" to reject one real mechanism (`max_retries=`) while nearly relying on an even-less-documented one (`condition=` counter comparisons) — see `ADR-011`'s Context. The boundary held, corrected before landing, once this term's referent was pinned to one concrete file rather than a vague adjective |
| Compile rate | The fraction of non-`irreducible` acceptance criteria that compile to a `parallelogram` (real, deterministic) acceptance gate rather than falling back to a `box` (self-report) one. Distinct from `FR-3`'s "has a compiled-check entry," which both gate shapes satisfy. | `prd.md` `OQ-18`; `architecture.md`'s Gate-preference text and Risks row 3; `roadmap.md` Phase 2/4 | — | none yet — first named precisely during `/delivery:roadmap`'s review, after "compile rate" and "has a compiled-check entry" were found colliding under one loose phrase |
| Ambiguously observed | A real governed call the hook saw but could not attribute to exactly one `.delivery/` — the session ran from a cwd where several were reachable, so the record went into every candidate, marked, instead of being silently dropped. Weaker than **Invoked**, categorically different from a **Not-invoked** artifact's nothing. | `context-management/brief.md` F-1/F-2; the 2026-08-10..14 blackout (gy5.1); `skills/status/SKILL.md`'s fourth state | `attribution: "ambiguous"` (`record-invocation.js`) | "observer silence" (the brief's working name for the whole failure class — the recorded state is this term; unrecorded silence is now a bug, not a state) |
| Memory surface | Any store an agent may read or write across sessions. This project has three: bd (beads — primary for tasks and working memory, by operator decision), Claude Code auto-memory (fallback only, when bd is unavailable), and `.delivery/` (the artifact store — a record, not a memory surface, kept distinct deliberately). | `context-management/brief.md` OQ-2; repo `CLAUDE.md` beads block | — | "memory" bare, where it could mean the harness feature or the plugin's persistence — always name the surface |
| Compaction | The harness summarizing or trimming a session's context when it nears its limit; invoked skills survive it only within a budget, everything conversational may be summarized away. A harness behavior the plugin cannot gate — only write instructions that survive it. | `context-management/baseline.md` census (one event in 34 sessions); gy5.5's context-integrity rules | `compact_boundary` (transcript marker) | — |
...
| 2026-08-13 | Added `Documented interface` | `attractor-handoff` architecture phase (feature-critic review) found the word "documented" doing real design work across three artifacts with no fixed referent — exactly how a real double standard (rejecting one real attractor mechanism while nearly relying on a less-documented one) got through unchallenged | `attractor-handoff/brief.md`, `prd.md` predate this entry, used the word loosely — no fix needed, neither made a decision that turned on the ambiguity; `architecture.md`/`ADR-011` are the first documents written under the precise term |
| 2026-08-13 | Added `Compile rate` | `attractor-handoff` roadmap phase (feature-critic review) found `OQ-18`'s "compile rate" colliding with `FR-3`'s unrelated "has a compiled-check entry" — the roadmap had tied `OQ-18`'s measurement to a cuttable stretch item that doesn't actually produce it | `prd.md` `OQ-18` predates this entry, used the phrase loosely — no fix needed there; `roadmap.md`'s Phase 2/4 split is written under the precise term |
| 2026-08-27 | Added `Ambiguously observed` | `context-management` gy5.2: the invocation hook's ambiguous-cwd decline branch wrote nothing, making a dead observer indistinguishable from an idle session for 14+ days (the 08-10..14 blackout, root-caused in gy5.1); the fix records the ambiguity, and the state needed one name before status, the hook and the brief each coined their own | `harden` artifacts and `skills/status/SKILL.md` predate the term; status updated same session, harden's realign carries an addendum |
| 2026-08-27 | Added `Memory surface`, `Compaction`; `Session` revisit deferred with F-11 (still excluded — session-boundary state enumeration is out of this initiative's scope, so Session stays a harness word for now); noted `context-management/brief.md` r1 was written under the pre-`Ambiguously observed` vocabulary and revised same day (r2) | R-cm-11: the brief used all three words without entries — the exact quiet-introduction the glossary forbids | brief r1 (`198545e`..`7fa20ae`) used "observer silence"; r2 adopts the glossary term |
