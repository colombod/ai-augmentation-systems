...
| Self-correction check | A `/delivery:status` or `/delivery:challenge` run — an independent look at the work, separate from the person doing it. | Finding D | — | "status/challenge-equivalent check" (used in the PRD draft — replace with this term there) |
