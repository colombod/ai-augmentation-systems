...
prd.md/architecture.md set for this epic. This story reproduces FR-24/25/27's exact text, the
full S-7 Case table, an Interface convention this story itself must fix (Interface 1 says so
...
title: Bounce agent-invented scope back to its originating agent (S-7)
status: ready
epic: chief-of-staff
supersedes: []
superseded_by: []
...
requirements: [FR-24, FR-25, FR-27]
depends_on: [chief-of-staff-03]
size: M
---

# S-7: Bounce agent-invented scope back to its originating agent
...
staff has triaged it" (glossary) — first goes through S-6's citation check. Only a question S-6
cannot answer directly (no exact, nameable source settles it) falls through to S-7. This story
implements that fall-through check and its bounce/escalation behavior only — S-6's own citation
...
**Why S-7 stays narrow (from `prd.md`'s own S-11 comparison, quoted so this story's boundary is
explicit, not inferred):** "S-7 is reactive and narrow — passes anything that traces to a
stated requirement, full stop." A requirement scoped loosely enough to permit drift is still a
pass for S-7 — that broader mission-alignment check is S-11, a separate scenario and a separate
...
> S-6/S-7/S-8 exist to intercept... `business-analyst.md`'s existing Open Questions convention
...
> from S-6's "candidate question that would otherwise surface directly to the operator." ...
> `solution-architect.md`'s existing spike-flagging habit already *is* S-8's own mechanism
> natively.
...
story. Nothing in this story routes those writes through S-7 — they were never candidate
questions in S-6's sense to begin with.

## Files and modules

| Path | What to do |
| :-- | :-- |
| `agents/chief-of-staff.md` | modify — add the S-7 branch: for a candidate question S-6 could not answer, check whether any stated requirement (FR, scenario, architecture decision, story AC) traces to it by citable traceback; if none does, construct the `bounced` outcome (originating agent name + missing requirement); if it does, do not classify as agent-invented and let the question continue past S-7 unaffected; handle the three escalation cases (provenance unknown, twice-bounced, disputed) by writing a queue entry per Interface 3 |
...
bounced   — S-7, with originating agent and missing requirement   <- this story
spiked    — S-8, with the spike story's path
queued    — S-9, with the queue-entry ID and its Blocking flag
```
...
which of S-6/S-7/S-8 it believes applies — then says explicitly: "this is a convention to
specify at story-time (`delivery-lead`), not a contract to lock here." That specification is
this story's own job, and FR-25 cannot be met without it: **the consultation prompt must also
state the calling agent's own identity** (its agent/role name — e.g. "I am
`solution-architect`, consulting about..."). That field is what the S-7 branch reads to
...
row here with `Source: S-7`:**

```
Columns: ID, Rank, Blocking (y/n), Source (S-6/S-7/S-8), Item,
...
about the same output and merges rather than duplicating (`FR-52`'s structural half, shared
plumbing this story reuses, does not rebuild).

**Full S-7 Case table, `prd.md` verbatim:**
...
- **ADR-002** — chief of staff is a real subagent invoked via the Agent tool, plus a thin skill
  wrapper and per-agent pointer sections. The S-7 classification and bounce-message logic lives
...
- **Epic framing (`prd.md`'s epic intro)** — chief of staff protects "the human principal's
  attention and the mission they actually stated." S-7 is the narrow half of that protection
  (traceability at the level of one question); this story implements only that half, not S-11's
...
- [ ] Case table — originating agent can't be identified: escalates to S-9 marked "provenance
...
- [ ] Case table — originating agent disputes the bounce: chief of staff does not adjudicate the
  scope question itself; it routes to S-9 as a scope dispute, stating the dispute, not a ruling.
- [ ] A compound candidate question where part traces to a stated requirement and part does not:
  ...
  literal `prd.md` S-7 Case-table row — derived by applying `FR-24`'s citable-traceback check at
  sub-question granularity, the same way `FR-31` splits a mixed S-8 question. Flagged here so
...
- [ ] A question that does trace to a stated requirement, however loosely scoped, is not
  classified as agent-invented and is not bounced — per the S-11 comparison quoted in Context,
  S-7 passes anything that traces, full stop.
- [ ] The bounce message and every escalation state the missing requirement or the dispute as
  ...
  non-goal, `prd.md` Out of scope: "Chief of staff overruling a scope decision itself (S-7)...
...
S-7 — do not fold them into one, the same mistake the architecture's first sketch made and
corrected.
- Classification + messaging (`FR-24`/`FR-25`): **example-based.** Risk: High — "same silent
  misclassification risk class as S-6, one hop later."
- Escalation paths (`FR-27`, Case table): **integration, fixture-driven.** Risk: Medium —
  "self-limiting (S-9 catches it eventually)."
...
| Originating agent cannot be identified (self-identification field absent/unresolvable) | Escalates to S-9's queue marked "provenance unknown" |
| Same question bounced twice on record, still unresolved | Escalates to S-9 rather than bouncing a third time |
| Originating agent disputes a bounce | Routed to S-9 as a scope dispute; chief of staff records the dispute, does not adjudicate |
| Two agents independently invent overlapping/near-duplicate scope | Not tested — explicit MVP-1 non-goal (semantic dedup; exact/near-exact matching is S-8/S-12's job) |
...
escalation actually surfaces correctly inside S-9's assembled briefing needs `chief-of-staff-08`
to exist — that's the escalation *destination*'s own acceptance criteria, not this story's. What
this story verifies directly, without it, is that the correct queue entry (Source: S-7, right
...
- **Semantic dedup of overlapping invented scope** — `prd.md` Case table, verbatim: "Two agents
  ...
  disproportionately hard; exact/near-exact matching only (S-8/S-12)."
- **Bypass detection** — `prd.md`'s own "Documented constraint, not an AC," verbatim: "nothing
  ...
  out of scope (bypass ≠ unavailability/`FR-51`)." `FR-51` (unavailability fallback) is a
...
- **S-11's mission-drift check** — separate scenario, separate story (Phase 7b). This story
...
- **`chief-of-staff-08`** (S-9 briefing assembly, Phase 8) — needed only to verify, end-to-end,
  that an escalated item (provenance-unknown / twice-bounced / disputed) actually surfaces
  correctly inside S-9's assembled briefing. That is the escalation *destination*'s own
