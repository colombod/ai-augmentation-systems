# ADR-006: Version-lifecycle mechanics are agent-judged prose, not code
...
The PRD (`initiatives/document-lifecycle-versioning/prd.md`, `FR-1`–`FR-17`) needs the
same-problem test, the Version-history table/heading-count consistency check (`FR-14`),
and staleness-by-version-date (`FR-10`) evaluated correctly on every relevant skill run.
This plugin ships zero runtime code for any `.delivery/` document convention today, except
the Version 1 invocation ledger (`hooks/scripts/record-invocation.js`), which exists
specifically because tool-call history is the one signal an agent cannot narrate around
(`ADR-001`) — a different problem (detecting whether an action happened) from this one
(comparing two pieces of prose, or counting headings in a Markdown file small enough to
read in full: `NFR-3` caps the volume at roughly six version rows per document, across
seven documents).
...
**Why rejected:** the volume this ever needs to handle (`NFR-3`) is a read well inside
direct agent capability. A script becomes a second definition of "well-formed" that must
stay synced with the prose rule in `version-history.md` — the exact drift risk `ADR-007`
...
**Why it was attractive:** would make `FR-2`/`FR-5` ("result and reasoning written into
...
ordinary conversation, a materially larger change than the PRD asked for.
...
**We will need to revisit this if:** a document ever exceeds `NFR-3`'s ~6-version
