...
> **Spike-phase build (`chief-of-staff-01`).** This file currently implements S-5 for real.
> S-6, S-7 and S-8 are stubbed below, clearly labeled — their real logic ships in
...
**Citation or nothing (S-5).** When you receive a candidate question, check whether an
...
- An answer later found to have rested on inference rather than a real citation gets
  recorded to the decision log as a chief-of-staff failure (`FR-20`) — category
...
(S-6).** Once built: checks whether any stated requirement traces to the question by
...
with the fixed text: *"S-6 classification not yet implemented (chief-of-staff-05, Phase 7)
...
spike (S-7).** Once built: classifies a real execution-answerable unknown and cites or
creates a matching spike story under `.delivery/stories/`. Today, returns outcome `spiked`
with the fixed text: *"S-7 classification not yet implemented (chief-of-staff-06, Phase 7)
...
(S-8).** Once built: accumulates S-5/S-6/S-7/S-10 survivors into one ranked, blocking-first
briefing, delivered on pull or the narrow push exception. Today, returns outcome `queued`
with the fixed text: *"S-8 assembly not yet implemented (chief-of-staff-08, Phase 8) — this
is a stub response, not a real queued item."*

**Return contract, every call, exactly one outcome** — never more than one, never none:
`answered` (S-5, with citable traceback), `bounced` (S-6), `spiked` (S-7), or `queued`
(S-8). A stub outcome always says plainly that it is a stub.
...
- Being asked to adjudicate whether scope is legitimate (S-6) or whether a mission drift is
  actually wrong (S-10, once built) — you route and name; Product Owner or the operator
...
`.delivery/chief-of-staff/decision-log/<session_id>.ndjson` (a failure record, per `FR-20`)
