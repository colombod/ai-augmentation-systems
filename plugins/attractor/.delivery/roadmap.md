...
**What is not yet resolved:** two narrow Solution Architect decisions the architecture's own
Risks table names explicitly (component-node FAIL routing; branch-rejection handling — see
work item A below and Dependencies outside our control). Both are scoped tightly enough to
close inside this phase, not a reason to defer starting it — seven of this phase's other nine
work items (B through H) do not depend on either answer; only the final two (I, the integration
point, and J, which depends on I) do.
...
| Work item | Size | Confidence | Depends on |
| :-- | :-- | :-- | :-- |
| **A** — Two narrow Solution Architect decisions, both already named blocking in `architecture.md`'s own Risks table, neither new research: (a) does a `PARALLEL` node's join-FAIL outcome route via the ordinary fail-edge/`retry_target` ladder (Answer 5's own wording) or via the unconditional convergence-node jump (attributed to "the Component-structure diagram's wording," though that literal phrase describes amplifier's reference `engine.py`, not the current TS design's own "Component structure" diagram, which was revised by the fifth pass and states the main loop's handling of a `PARALLEL` dispatch as "otherwise unchanged... an ordinary `Handler` call from the loop's point of view" — the design as written still states both readings are live, which cannot both be true for the same run); (b) does `ParallelHandler.execute()` wrap each branch's `runBranch` call so a rejection becomes that branch's own FAIL `Outcome` (closes the orphaned-sibling-worktree hazard `Promise.all`'s reject-and-abandon semantics create today), or does the dispatch switch to `Promise.allSettled` with an explicit reject-to-FAIL mapping — either resolves it, neither is written down yet. One paragraph each, same shape as ADR-006(b) in Phase 1. **A(a) is not purely a documentation decision — see item C below:** if it resolves toward the unconditional-jump reading, implementing it requires mutating the loop-local `currentId` inside `core/engine.ts` directly, something only that file's own loop can do, not `handlers/parallel.ts`, which reopens item C's "closed, refactor-only" exit criteria | S | high | none |
...
| **C** — `Engine#executeNodeStep` extraction (ADR-012, resolves F5+F6): `run()`'s existing loop (`engine.ts:643-1088`) is refactored onto one shared private method plus a new `stepCount` instance field, replacing the loop-local `step` variable. `runBranch` does not exist yet after this item — that is item F's job; this item's own exit condition is the *existing* test suite passing unchanged against the refactored `run()`, since it is a refactor of already-shipped, tested code, not an additive change. **This "closed, refactor-only" status is conditional on how item A(a) resolves, not settled independently of it:** if A(a) resolves toward the unconditional convergence-node-jump reading, implementing that jump means mutating the loop-local `currentId` inside `core/engine.ts`'s own `run()` loop directly — a change only this file can make, not an addition to `handlers/parallel.ts` — which reopens this item's exit criteria as an additive change, not a pure refactor. Start C now regardless (it is not blocked on A), but do not mark it permanently closed until A(a) is answered | L | medium | none |
| **D** — `Backend.run()` cwd parameter (ADR-008): additive trailing param on `Backend.run()`; `ClaudeCodeBackend` prefers the per-call value over its constructor-bound one; `BoxHandler.execute` passes `ctx.cwd` through (today it does not — confirmed `box.ts:96-102`) | S | high | none |
...
lowest confidence rating on this board — precisely because it is the one item still blocked on
work item A's two open Solution Architect decisions. **No inversion check possible on that one
item until A resolves;** every other item's size/confidence pair is a genuine independent
...
| Two Solution Architect decisions inside the FR-17b architecture itself — component-node FAIL routing (ordinary fail-edge/`retry_target` ladder vs. the unconditional jump-to-convergence the Component-structure diagram also describes) and branch-rejection handling (catch-and-convert each branch to a FAIL `Outcome` vs. switch `ParallelHandler`'s dispatch to `Promise.allSettled`) | Solution Architect | Before Phase 5's item I (`ParallelHandler`) can be implemented, and before the "Component-node FAIL routing" and "Branch throws mid-flight" test rows can be written | Open — both flagged blocking in `architecture.md`'s own Risks table | Items B–H (7 of Phase 5's 10 work items) proceed unaffected; only item I and its two dependent test rows stay blocked |
...
| **Phase 5.** Component-node FAIL routing: the architecture's own text gives two different answers for what happens when a `PARALLEL` node's join outcome is FAIL (ordinary fail-edge/`retry_target` ladder vs. an unconditional jump to the convergence node) | confirmed present in the design as written | high — whichever is right, the first author whose fan-out actually fails hits the untested path, and a fan-out failing is the entire reason a zero-success-checking join policy exists | Work item A resolves this before item I (`ParallelHandler`) is implemented; the "Component-node FAIL routing" test row is written the moment it is | Solution Architect |
| **Phase 5.** `ParallelHandler`'s branch dispatch is `Promise.all`-based; if one branch's `runBranch` call rejects (not a FAIL `Outcome` — an actual throw, e.g. a worktree name collision) rather than resolving, `Promise.all` abandons the still-pending siblings — their settlement, worktree removal, and branch checkpoint are all orphaned, and the rejection escapes `Engine.run()`'s public API uncaught | medium — worktree creation failing on a name collision, or a backend crash, are both plausible sources | high — orphaned worktrees and an engine that can throw instead of returning a `RunResult` are the "silent degradation" class `AGENTS.md`'s doctrine forbids elsewhere in this design | Work item A's second decision (catch-and-convert to FAIL vs. `Promise.allSettled`), implemented in item I, verified by item J's branch-throws test | implementer, decision needed from Solution Architect |
