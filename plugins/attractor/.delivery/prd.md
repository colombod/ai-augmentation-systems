...
| S2 | Human gate blocks; how it's answered is unsettled | P-2 | 2 criteria confirmed build-agnostic, 1 reframed as a non-goal check, 1 (FR-8) blocked on reconciling two competing designs — see Open question 1. Whether this serves P-2's walk-away need, and how far NFR-9's crash-during-wait risk narrows, depends on which design wins |
...
| FR-8 | *(blocked — see Open question 1)* Once the channel-vs-park/resume architecture question is reconciled, an answer reaches the gate and takes the matching edge. Two candidate designs exist (`.superpowers/specs/2026-08-05-human-gate-channels-design.md` and `.superpowers/specs/2026-08-03-attractor-claude-code-plugin-design.md` §7-8); neither is authoritative yet | S2 | must, once reconciled |
...
| NFR-9 | Unattended crash exposure during an unanswered human gate | A process death while blocked at S2's gate loses the entire run to that point. One candidate design (the 2026-08-05 channels spec) would narrow this to the `human` hop only; the other (the 2026-08-03 park/checkpoint spec) is designed specifically to survive this case via cross-restart resume. Which applies depends on Open question 1 | Accepted, undetected risk this slice — see Open question 1 and Non-goals |
...
- That FR-5's live-blocking `human` hop is interpretation-agnostic enough to survive either candidate resolution of Open question 1 — genuinely untested against the 2026-08-03 park/checkpoint design, which may replace rather than extend it. Do not build further on ADR-002 alone until Open question 1 is settled.
- If the 2026-08-05 channels design (or some hybrid) survives reconciliation: that `--allow-agent-gates` defaulting to off, requiring both the graph's and the operator's opt-in, is the right trust boundary for the `agent` channel specifically — a deliberate, conservative choice, not yet tested against a real operator's workflow, and contingent on that design surviving at all.
...
| :-- | :-- | :-- | :-- |
| 1 | **UNRECONCILED, not resolved — corrected 2026-08-05.** A channel-abstraction design (`.superpowers/specs/2026-08-05-human-gate-channels-design.md`) was written to answer this, then found to conflict with an existing, unread 2026-08-03 design (`.superpowers/specs/2026-08-03-attractor-claude-code-plugin-design.md` §7-8) that already specifies park + checkpoint + cross-restart resume via `attractor resume`/`attractor approve`, not an in-process channel abstraction. Comparison and candidate resolutions in `.superpowers/carry-forward.md` under "Plan 4." FR-8 remains blocked on this being reconciled, not on the channel design alone. | Product Owner | FR-8 |
| 2 | **Same unreconciled state as Q1** — §7.4 of the 2026-08-03 design already specifies an `--unattended` preflight rejecting any reachable gate lacking `on_timeout`, a similar but not identical condition to the channel design's "no viable hop." Resolves once Q1 does. | Product Owner | S2 scope |
