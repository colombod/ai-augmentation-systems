...
> Status: reviewed · Last updated: 2026-08-07
...
> place, nothing else rewritten. One row in the Test strategy table cannot be written yet: it
> surfaces a genuine self-contradiction in the design's own routing answer that needs the
> Solution Architect's resolution first (see Risks).
>
...
1. **Integration into `run()`.** Not a nested `new Engine(...)` (rejected — see ADR-009).
   **Revised 2026-08-07 (F5, F6 — ADR-012, binding):** the main loop and a new private
   ...
   bookkeeping needs no special case. **What this revision does NOT resolve:** the existing
   "QA finding, blocking" Risks-table row (unresolved component-node FAIL routing — whether a
   FAIL join outcome routes via the ordinary fail-edge/`retry_target` ladder or via an
   unconditional jump) is **not** one of F1–F7 and is left exactly as open as it was; this
   refactor does not take a side on it, and neither callers of `executeNodeStep()` are written
   to assume an answer either way.
...
| **Component-node FAIL routing** | **unresolved — see Risks, blocking** | **cannot be written yet** | The design's own text disagrees with itself: does a join=FAIL outcome route via the ordinary fail-edge/`retry_target` ladder (answer 5's wording), or does the "PARALLEL with ≥2 outgoing edges skips `selectEdge()`" jump (Component structure diagram) fire unconditionally on status too? One sentence of resolution unblocks this row |
...
| **QA finding, blocking.** The design's own text gives two different answers for what happens when a `PARALLEL` node's join outcome is FAIL: answer 5 ("Retry inside a branch") says the existing §3.7 fail-edge/`retry_target` ladder routes it, the "Component structure" diagram says the new jump case ("PARALLEL with ≥2 outgoing edges skips `selectEdge()`, sets `currentId` = convergence node") fires unconditionally on having ≥2 outgoing edges, with no status check written into that sentence. These cannot both be true for the same run | confirmed present in the design as written | high — whichever is right, the FIRST author whose fan-out actually fails hits the untested one, and a fan-out failing is not a rare event (it is the entire reason a default join policy that checks for zero successes exists, per Open Question 5) | State explicitly which one wins before implementation; the "Component-node FAIL routing" row in Test strategy above is written the moment this is answered, not before | Solution Architect |
| `ParallelHandler.execute()`'s branch dispatch is named explicitly as `Promise.all`-awaited (Spike 8's own wording, not a QA inference). If any branch's `runBranch` call *rejects* — as opposed to resolving with a FAIL `Outcome` — `Promise.all` rejects on the first rejection and does not cancel or continue awaiting the still-pending sibling branches. Their eventual settlement, worktree removal, and branch checkpoint all become orphaned inside that one `ParallelHandler.execute()` call, and the rejection propagates out of `Engine.run()`, a public API surface `cli.ts` does not currently handle as anything but a FAIL `RunResult` | medium — worktree creation failing on a name collision (Spike 10's own concern) is exactly this kind of throw, not a FAIL Outcome, and a backend crash is another plausible source | high — orphaned worktrees and an engine that can throw instead of returning a `RunResult` are exactly the "silent degradation" class `AGENTS.md`'s doctrine forbids elsewhere in this design | Wrap each branch call so nothing thrown escapes the `Promise.all`/`allSettled` boundary uncaught — convert to that branch's own FAIL `Outcome` instead — or switch to `Promise.allSettled` and treat a rejection as a FAIL result explicitly. Neither choice is stated anywhere in the design as written; the "Branch throws mid-flight" test row above cannot be locked down until one is | implementer, decision needed from Solution Architect |
