...
a branch root reachable from a sibling root, or one that merely lost a depth
tie for it), or reachable from a single branch's own shortcut into the
convergence node's own downstream territory.
