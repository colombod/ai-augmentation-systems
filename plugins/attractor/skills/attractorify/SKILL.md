---
name: attractorify
description: Analyze the current session and decide whether an attractor pipeline is warranted -- then design one conversationally if it is. Triggers -- "/attractorify", "should this be an attractor?", "design a pipeline for", "attractorify this", "do I need an attractor pipeline?", "turn this into a pipeline". Use when the user wants to author a new .dot pipeline from a plain-language description, not when a .dot file already exists (use the attractor skill's lint/run/doctor commands for that).
---

# /attractorify — Attractor Pipeline Design Skill

Ported near-verbatim from `microsoft/amplifier-bundle-attractor@main`'s
`skills/attractorify/SKILL.md` (2026-08-10) — the three-question test, the
...
execution with no session context would see nothing of the session it is supposed to
analyze.
...
Apply the three-question test from
...
- The `budget:` line is required when the verdict is `attractor` or
  ...
  **machine-encoded in the recommended invocation** (`--param` values, or a
  documented iteration cap) — `budget: N/A` alongside an emitted command is a
...
[`reference/pipeline-design-principles.md`](reference/pipeline-design-principles.md) §0,
and stop — the artifact is the record of why, and these verdicts emit **no pipeline
invocation**, so they end here, outside the design gate. Do not follow a "no" with
leading questions hunting for a cycle ("want automatic retry on failure?") — a cycle you
have to fish for is not in the work, and assent to a question you posed can never be
quoted as evidence (rules above; the verifier checks exactly this).

`existing-attractor` (reuse a shipped exemplar — see
[`examples/00-convergence-loop.dot`](examples/00-convergence-loop.dot) or
[`examples/04-retry-with-fallback.dot`](examples/04-retry-with-fallback.dot)) is **not
an exit**: it hands the user an invocation command, which is a design handover in every
way that matters. It carries the same artifact discipline (three questions with quotes,
`counter:` line, `budget:` line) and must pass the executed gate AND clear independent
diagnosis-verification below (its one round — or an override, which skips it) before any
command is emitted.
...
The gate fails **closed** — this mirrors the engine's own fail-closed goal-gates
(`engine-semantics.md`): a defaulted, absent, provisional, or quote-free verdict never
satisfies it. On FAIL, produce no design; go back to the diagnosis or to clarifying
questions. Do not weaken the check to fit the artifact; fix the artifact, or honestly
stop. A PASS authorizes only the next step: this gate verifies **form, not truth** —
independent diagnosis-verification below is what authorizes design.

### Independent diagnosis-verification — after form-PASS, before ANY handover

A bash gate cannot tell a real quote from a relevant one, and the judgment that authors
the evidence is the same judgment that folds — so certification is taken away from the
authoring judgment. Once the artifact is written and the form gate passes — unless a
valid `override:` line is recorded, which ends adjudication (rule above): overridden
verdicts skip this section entirely — and BEFORE any handover, you MUST delegate
verification to a **fresh-context subagent with no stake in the design**: use the Task
tool with `subagent_type: general-purpose` (or an unspecified default — the isolation
property is what matters, not a specific agent type), NOT `attractor:attractor-expert` —
a plain clean-slate delegation that sees only what the instruction below carries. That
isolation is the point, and it is a different gate from the execution-verification step
later in this skill (Step 3, item 6) — **this one checks whether the reasoning to build
a pipeline was sound; that one checks whether the resulting graph actually runs.**
...
**Fallback (Task tool unavailable):** answer the three verifier questions in
...
evidence (the same anti-self-dealing rule as above). A reversal requires
**rewriting `.attractorify/diagnosis.md` with new user-originated evidence,
then re-running the gate AND the verifier** (a rewritten diagnosis with new
evidence gets its own single round) — social pressure produces no quotes, so
an artifact that could not pass before the pushback still cannot. A human
approval/sign-off step is not a machine-checkable gate: it belongs on the
`counter:` line, not engineered around.
...
1. **The TARGET** — which repo / working directory. Asking is
...
3. **The BUDGET caps** — a node cap AND, if the pipeline can loop, a
   `max_retries`/`default_max_retry` cap; a guessed cap is a guess like any other.

**Each answer must land machine-visibly in the deliverable.** The chosen
target becomes a working directory / path / `--param` in BOTH the pipeline's
...
become node/graph attributes or invocation `--param`s.
...
the independent diagnosis-verifier returned `VERIFIER: VALID`; a valid `override:`
...
2. **Consult the `attractor-expert` agent — by delegation, not by reading its
   file.** Delegate to `attractor:attractor-expert` (the Task tool) at design start
   (pattern and shape choice), for any mid-build routing or engine-semantics question, and
   ...
   guide; a generic builder carries no attractor engine semantics. If delegation
   is unavailable, fall back to reading `../../agents/attractor-expert.md` directly.

3. **Follow design order** from
   [`reference/pipeline-design-principles.md`](reference/pipeline-design-principles.md) §0:
...
4. **Fit the declared budget.** Keep the control plane lean
   (`pipeline-design-principles.md` §0, control-plane vs recipe-plane
...
6. **Write the artifact and lint it.** Write the `.dot` file to a named path in
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/dist/attractor.js" lint <path>
   ```
   A `.dot` that fails this lint (see [`README.md`'s Lint rules](../../README.md#lint-rules))
   is not a runnable artifact. Fix lint findings before proceeding — including any
   `HAND-001` finding, which means step 5 was violated and the graph needs a different
   handler, not a suppression.

7. **Execution-verification (delegated, independent — FR-13).** See
   [ADR-017](../../.delivery/decisions/ADR-017-delegated-execution-verification.md).
   Before this graph may be handed back as `ready`, a fresh-context Task-tool subagent
   — separate from and in addition to Step 1's diagnosis-verifier — must actually run
   it and report a real transcript. Full procedure in `verify-run.ts`'s own header
   comment and the delegation instruction template there; the short version: launch a
...
8. **Hand back:** the `.dot` file path, the exact invocation command, the literal
   ...
   `--live`), and a one-line summary of why each structural choice was made. The chosen
   target must appear as a working directory / path / `--param` in both the graph's
   deterministic gate commands and the invocation, and any budget caps must be
   machine-encoded in graph attributes or invocation parameters — scoping or a budget
   stated only in prose is neither.
...
- `attractor:attractor-expert` — the consultable expert agent; delegate to it
  ...
  `../../agents/attractor-expert.md`; read directly only when delegation is
  unavailable)
- [`examples/00-convergence-loop.dot`](examples/00-convergence-loop.dot) — canonical
  control-plane skeleton, ported and executed on this engine
- [`examples/04-retry-with-fallback.dot`](examples/04-retry-with-fallback.dot) — shipped
  exemplar for an `existing-attractor` recommendation
...
- **Not an auto-runner.** It hands back the artifact, the invocation, and the
  execution-verification transcript; launching a real (`--live`) pipeline run is the
  human's explicit call.
- **Not a doctrine memoir.** It links the shipped reference docs for depth; it does not
...
- **Not a channel to an unregistered handler.** It will never generate, and will never
  hand back as `ready`, a graph using `hexagon`/`tripleoctagon`/`house` — see step 5
  above and [`dot-reference.md`](reference/dot-reference.md).
