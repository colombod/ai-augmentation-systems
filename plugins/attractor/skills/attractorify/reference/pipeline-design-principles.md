...
> `docs/PIPELINE_DESIGN_PRINCIPLES.md` (2026-08-10) — confirmed engine-independent design
> doctrine. Two corrections applied throughout, per
> [ADR-018](../../../.delivery/decisions/ADR-018-reference-material-porting-split.md):
> every `model_stylesheet`/`llm_model` reference is stripped (PRD non-goal), and every
> citation to an amplifier example this project did not port
> ([ADR-019](../../../.delivery/decisions/ADR-019-example-portability-policy.md)) is
> repointed to this project's own equivalent or removed. Section headings and order are
...
descend toward a verified goal state. `../examples/00-convergence-loop.dot` (this
project's own canonical skeleton exemplar, ported and executed — see the
Example-portability policy in `architecture.md`) is the minimal case: a gate and a
corrective back-edge, no domain phases, pure control-plane responsibility.
...
`../examples/01-simple-linear.dot` is the deliberate one-pass shape;
...
`../examples/02-plan-implement-test.dot` deliberately uses staged nodes
...
reuse a shipped exemplar (`../examples/00-convergence-loop.dot`) — not a verdict.
...
depend on whether the box node actually wrote its artifact. Live catch (amplifier's
own production history): a report-writing node returned SUCCESS without writing its
report twice in consecutive runs; a deterministic gate was added after the second
occurrence — an `[ -s report.md ]` non-empty check that writes a labeled stub when the
report is missing, immediately after the node that was supposed to produce it.
...
[`../examples/04-retry-with-fallback.dot`](../examples/04-retry-with-fallback.dot).
...
discipline from [`pipeline-patterns.md` §AP-2](pipeline-patterns.md): have the LLM write
...
iteration produces refusals on mechanically-green work — amplifier's own production history
records 6 consecutive critique refusals occurring before a stall counter was added to the
outer quality loop. Route stall detection to escalation, not to infinite retry; anchor
critique to the DoD plus prior acceptances so the quality bar ratchets rather than resets.
...
that the environment is in the expected state before the corrective loop starts — amplifier's
own production history records 14 red iterations burned re-flipping a coin on a stale-cache
defect that no code change could fix. An attractor absorbs model drift, not deterministic
bugs; the loop cannot converge on a defect that lives outside the graph's reach.

**Add transient fail-routes on every ship-path node.** Ship-path LLM nodes (critique,
feedback, package) should carry `outcome=fail` edges back to a recovery point — amplifier's
own production history records a transient provider error at a ship-path node killing a
whole run after the work was already done. One edge addition fixed it permanently.
...
check — a missing file should fail the gate, not silently pass it.
...
For DOT skeletons for each strategy, see [`pipeline-patterns.md`](pipeline-patterns.md).
...
Attractor graphs support `$param`/`${param}` expansion via `--param key=value` at
invocation. When a design choice — iteration count, quality threshold, verbosity mode —
...
should be able to run the pipeline unchanged and get a correct result. (This engine has
no graph-level `params=` declaration or `default_*` scaffold beyond `default_max_retry`
— an author states the default inline in the node/edge attribute itself, e.g.
`max_retries="3"`, or documents the expected `--param` in the pipeline's own guide.)
...
Operators pass `--param goal=... --param quality_threshold=...` to tune behavior; the
pipeline handles the common case without them via `default_max_retry`.
...
**The incident (amplifier's own production history, 2026-07-28).** A 20-node pipeline ran
2.4h and exited success with zero work product. The per-slice test gates ran a test suite
green on an **unmodified tree** — green tests certify the tree's state, not that new work
exists in it. The pipeline could not distinguish "the work was never written" from
"concurrent agents clobbered uncommitted work" (~29 overlapping agent sessions in the same
repo during the final hour) — precisely because nothing was committed. A delta-assertion
gate makes that ambiguity structurally impossible: durable commits or the gate is red.
...
Amplifier's own incident retrospective found that the preflight vantage gates which DID
work correctly wrote truthful results to a durable env file before work began. The same
discipline applies to any environment check:
...
Amplifier ships a growing copy-paste `.dot` snippet library at `examples/gates/` for
these patterns. **Not ported this slice** — this project's own gate primitives live
inline in this section (Patterns A/B/C above) and in the ported worked examples under
`../examples/`, not as a separate snippet library. Revisit if a dedicated gate-primitive
library earns its own place later.
...
| Topic | Where to look |
|---|---|
| Implementation patterns for LLM/code-tier nodes, SF/V+R/anti-patterns | [`pipeline-patterns.md`](pipeline-patterns.md) |
| Node shapes, handler attributes, edge conditions | [`dot-reference.md`](dot-reference.md) |
| Conditional routing edge selection algorithm | [`routing-reference.md`](routing-reference.md) |
| Retry-with-fallback and convergence example | [`../examples/04-retry-with-fallback.dot`](../examples/04-retry-with-fallback.dot) |
| Engine-level contracts (substitution, fail-fast, routing) | [`engine-semantics.md`](engine-semantics.md) |
| Delta-assertion gate primitives (base-SHA anchor, preflight evidence) | Patterns A/B/C above — no separate snippet library this slice |
